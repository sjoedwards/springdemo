...
public class DemoSecurityConfig {
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		http.authorizeHttpRequests(authorize -> authorize.anyRequest().authenticated())
				// will resolve controller route showMyLoggingPage initially, and the post will call authenticateLogin - this route is built into spring! PermitAll means anyone can see the login page
				.formLogin(form -> form.loginPage("/showMyLoggingPage").loginProcessingUrl("/authenticateLogin").permitAll());
		return http.build();
	}
